...
class Yaffs2Forensic:
    ...
    def list_obj (self):           # ← final user view
        """
        ...
        for object_id, header_versions in self.headers.items():
            ...
            if self.opt['last_only'] is not None and self.opt['last_only']:
                ...
                # name filter
                #==============================================
                if self.opt['name'] and header["name"] != self.opt['name']:
                    continue
                ...
                (assembled, sha1, data_offset) = self.get_data({
                ...
                if name:
                    allObj.append({
                        ...
                        "chunk_id":         header['chunk_id'],                 # NEW !
                        'visu_name':        header["visu_name"],                # NEW !
                        'index':            header['index'],                    # NEW !
                        'sequence':         header['sequence'],
                        'mode':             header['mode'],
                        'mode_brut':        header['mode_brut'],
                        "type_obj":         header['type_obj'],                 # NEW !
                        'size':             header['size'],
                        'uid':              header['uid'],
                        'gid':              header['gid'],
                        ...
                        'mtime':            header['mtime'],
                        'atime':            header['atime'],
                        "position":         header['position'],                 # NEW !
                        
                        "win_ctime_1":      header['win_ctime_1'],              # NEW !
                        "win_ctime":        header['win_ctime'],                # NEW !
                        
                        'parent_obj_id':    header['parent_obj_id'],
                        'offset':           header['offset'],
                        'data_offset':      data_offset,                        # NEW !
                        'obj_type':         header['obj_type'],                 # NEW !
                        'alias':            header['alias'],
                        'name':             name,
            ...
            ############################################################################
                ...
                for version_num, header in enumerate(header_versions, start=0):
                    ...
                    # object_id filter
                    #=================================
                    if not self.__is_object_id_selected(object_id, self.opt['obj_ids'], self.opt['obj_id_from'], self.opt['obj_id_to']):
                        continue
                    
                    #=================================
                    # version_num filter
                    #=================================
                    if not self.__is_version_selected(version_num, self.opt['versions'], self.opt['version_from'], self.opt['version_to']):
                        continue
    ...
    def get_data (self, opt):           # ← get data of an object id
        ...
        return assembled, sha1, retOffset
                ...
                try:
                        ...
                        if spare['chunk_id'] == 0:        # Header chunk
                            if spare['obj_type'] in self.OBJECT_TYPE_ID:
                                ...
                                self.headers[spare['object_id']].append({
                                    ...
                                    "win_ctime_1"   : dat['win_ctime_1'],
                                    "win_ctime"     : self.__format_time(dat['win_ctime_1']),
                            ...
                            if spare['object_id'] not in self.headers:
                                self.headers[spare['object_id']].append({
                                    ...
                                    "win_ctime_1"   : self.DEFAULT_CTIME,
                                    "win_ctime"     : self.__format_time(1),
                                ...
                                self.headers[spare['object_id']][0]['offset'] = position
                    ...
                    if self.debug >= 2:
                        if spare['obj_type_name'] is not None:
                            ...
                            # object_id filter
                            #=================================
                            if not self.__is_object_id_selected(spare['object_id'], self.opt['obj_ids'], self.opt['obj_id_from'], self.opt['obj_id_to']):
                                flag=False
                            #=================================
                            # version_num filter
                            #=================================
                            if spare['chunk_id'] == 0:      # Header chunk
                                if not self.__is_version_selected(len(self.headers[spare['object_id']]) - 1, self.opt['versions'], self.opt['version_from'], self.opt['version_to']):
                                    flag=False
                            else:                           # Data chunk
                                if not self.__is_version_selected(len(self.headers[spare['object_id']]), self.opt['versions'], self.opt['version_from'], self.opt['version_to']):
                                    flag=False
                                
                            #==============================================
                            # name filter
                            #==============================================
                            if self.opt['name'] and dat['name'] != self.opt['name']:
                                flag=False
